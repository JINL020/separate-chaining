...
class ADS_set {
   ...
   public:
    ...
    const_iterator z(size_t m) const {
        ...
        for (size_type i{0}; i < buckets; ++i) {
            if (values[i].head != nullptr) {
                return const_iterator(values + i, values + buckets, values[i].head, Mode::speziell, m - 1, values + i);
            }
        }
        return end();
    }
...
class ADS_set<Key, N>::Iterator {
   ...
   public:
    explicit Iterator(List *current, List *end, Entry *entry = nullptr, Mode mode = Mode::normal, size_type countdown = 0, List *begin = nullptr) : current{current}, end{end}, entry{entry}, mode{mode}, countdown{countdown}, begin{begin} {
    }
    ...
    //präfix!!!!!!!!!!!!!!!!!!!!!!
    Iterator &operator++() {
        if (mode == Mode::speziell && countdown == 0) {
            current = end;
            entry = nullptr;
            return *this;
        }
        ...
        if (mode == Mode::speziell && current == end && countdown != 0) {
            current = begin;
            entry = current->head;
            --countdown;
            return *this;
