...
class ADS_set {
   ...
   public:
    ...
    const_iterator y() const {
        ...
        for (size_type i{0}; i < buckets; ++i) {
            if (values[i].head != nullptr) {
                if (values[i].head->next != nullptr) {
                    return const_iterator(values + i, values + buckets, values[i].head->next, Mode::speziell);
                } else {
                    for (size_type j{i + 1}; j < buckets; ++j) {
                        if (values[j].head != nullptr) {
                            return const_iterator(values + j, values + buckets, values[j].head, Mode::speziell);
                        }
                    }
                }
            }
...
class ADS_set<Key, N>::Iterator {
   ...
   public:
    explicit Iterator(List *current, List *end, Entry *entry, Mode mode = Mode::normal) : current{current}, end{end}, entry{entry}, mode{mode} {
    }
