...
class ADS_set {
   ...
   public:
    ...
    size_t x() const {
        if (current_size < 2)
            throw std::runtime_error("less than 2 elements in set");
        ...
        for (size_type i{0}; i < buckets; ++i) {
            ...
            while (temp != nullptr) {
                ...
                if (std::less<key_type>{}(temp->data, min1->data))
                    min1 = temp;
                temp = temp->next;
            }
        }
        ...
        for (size_type i{0}; i < buckets; ++i) {
            Entry *temp{values[i].head};
            while (temp != nullptr && temp != min1) {
                if (first2) {
                    first2 = false;
                    min2 = temp;
                }
                if (std::less<key_type>{}(temp->data, min2->data) && temp != min1)
                    min2 = temp;
                temp = temp->next;
            }
        }
    ...
    friend bool
    operator==(const ADS_set &lhs, const ADS_set &rhs) {
