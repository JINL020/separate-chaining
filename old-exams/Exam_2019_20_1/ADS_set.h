...
class ADS_set {
   ...
   public:
    ...
    std::pair<key_type, key_type> x() const {
        ...
        Entry *saveFirst;
        Entry *saveLast;
        size_type saveCounter{1};

        for (size_type i{0}; i < buckets; ++i) {
            if (values[i].head != nullptr) {
                first = values[i].head;
                last = values[i].head;
                saveFirst = values[i].head;
                saveLast = values[i].head;
                break;
            }
        }
        for (size_type i{0}; i < buckets; ++i) {
            for (Entry *temp{values[i].head}; temp != nullptr; temp = temp->next) {
                if (key_compare{}(last->data, temp->data)) {
                    ++counter;
                    last = temp;
                } else {
        ...
        return std::pair<key_type, key_type>(saveFirst->data, saveLast->data);
    }
