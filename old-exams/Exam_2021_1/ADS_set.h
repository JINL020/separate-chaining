...
class ADS_set {
   ...
   public:
    ...
    iterator find(const key_type &key) const {
        ...
        while (temp != nullptr) {
            if (key_equal{}(temp->data, key)) {
                return Iterator(values + buckets, values + hashKey, temp);
            }
            temp = temp->next;
        }
        return end();
    ...
    const_iterator begin() const {
        for (size_type i{0}; i < buckets; ++i) {
            if (values[i].head != nullptr) {
                return const_iterator(values + buckets, values + i, values[i].head);
            }
        }
    ...
    const_iterator x() const {
        ...
        for (size_type i{buckets - 1}; i >= 0; --i) {
            if (values[i].head != nullptr) {
                Entry *currentEntry{values[i].head};
                while (currentEntry->next != nullptr) {
                    currentEntry = currentEntry->next;
                }
                return const_iterator(values + index, values + buckets, values + i, currentEntry, Mode::speziell);
            }
        }
    }
...
class ADS_set<Key, N>::Iterator {
   ...
   private:
    ...
    List *current;
    Entry *entry;
    Mode modus = Mode::normal;
    size_type counter = 0;

   public:
    explicit Iterator(List *end, List *current, Entry *entry = nullptr) : begin{nullptr}, end{end}, current{current}, entry{entry} {
    }

    Iterator(List *begin, List *end, List *current, Entry *entry = nullptr, Mode modus = Mode::normal) : begin{begin}, end{end}, current{current}, entry{entry}, modus{modus} {
    }

    Iterator() : begin{nullptr}, end{nullptr}, current{nullptr}, entry{nullptr}, modus{Mode::normal} {
    }
    ...
    Iterator &operator++() {
        ...
        if (modus == Mode::speziell) {
            if (begin + 1 == end) {
                std::cout << "only 1 element\n";
                ++counter;
                return *this;
            }
            current = begin;
