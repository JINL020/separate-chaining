   ...
   public:
    ...
    const_iterator y() const {
        if (current_size < 2)
            return end();
        for (size_type i{0}; i < buckets; ++i) {
            if (values[i].head != nullptr && values[i].head->next != nullptr) {
                return const_iterator{values + i, values + buckets, values[i].head->next, Mode::speziell};
            }
            if (values[i].head != nullptr && values[i + 1].head != nullptr) {
                return const_iterator{values + i + 1, values + buckets, values[i + 1].head, Mode::speziell};
            }
        }
    }
...
class ADS_set<Key, N>::Iterator {
   ...
   public:
    ...
    //präfix!!!!!!!!!!!!!!!!!!!!!!
    Iterator &operator++() {
        if (mode == Mode::speziell && counter < 1) {
            ++counter;
            return *this;
        }
        if (mode == Mode::speziell && counter == 1) {
