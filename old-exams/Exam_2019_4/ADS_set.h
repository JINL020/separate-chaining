...
class ADS_set {
   ...
   public:
    ...
    const_iterator w(const size_t *a) const {
        ...
        for (size_type i{0}; i < buckets; ++i) {
            if (values[i].head != nullptr) {
                return const_iterator(values + i, values + buckets, values[i].head, Mode::speziell, a);
            }
        }
        return end();
    }
...
class ADS_set<Key, N>::Iterator {
   ...
   public:
    explicit Iterator(List *current, List *end, Entry *entry, Mode mode = Mode::normal, const size_t *a = nullptr) : current{current}, end{end}, entry{entry}, mode{mode}, a{a} {
    }

    Iterator() : current{nullptr}, end{nullptr}, entry{nullptr}, mode{Mode::normal}, a{nullptr} {
    }
