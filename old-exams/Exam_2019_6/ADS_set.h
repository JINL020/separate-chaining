...
class ADS_set {
   ...
   public:
    ...
    key_type x() const {
        ...
        for (size_type i{0}; i < buckets; ++i) {
            for (Entry *temp{values[i].head}; temp != nullptr; temp = temp->next) {
                if (!define1) {
                    ...
                    if (std::less<key_type>{}(first->data, second->data) && std::less<key_type>{}(third->data, second->data)) {
                        max = second;
                        if (savedMax == nullptr || std::less<key_type>{}(savedMax->data, max->data)) {
                            savedMax = max;
                        }
                    }
                    first = second;
        ...
        if (savedMax == nullptr) {
            throw std::runtime_error("no local elem");
        }
        return savedMax->data;
    }
...
class ADS_set<Key, N>::Iterator {
   ...
   public:
    explicit Iterator(List *current, List *end, Entry *entry) : current{current}, end{end}, entry{entry} {
    }
