   ...
   public:
    ...
    const_iterator x(const key_type &a, const key_type &b) const {
        ...
        for (size_type i{0}; i < buckets; ++i) {
            for (Entry *temp{values[i].head}; temp != nullptr; temp = temp->next) {
                if (temp->data == a) {
                    return const_iterator(values + i, values + buckets, temp, Mode::speziell, last);
                }
            }
        }
    }
...
class ADS_set<Key, N>::Iterator {
   ...
   public:
    explicit Iterator(List *current, List *end, Entry *entry, Mode mode = Mode::normal, Entry *last = nullptr) : current{current}, end{end}, entry{entry}, mode{mode}, last{last} {
    }

    Iterator() : current{nullptr}, end{nullptr}, entry{nullptr}, mode{Mode::normal}, last{nullptr} {
    }
    ...
    //präfix
    Iterator &operator++() {
        if (mode == Mode::speziell && entry->data == last->data) {
            std::cout << "hello";
            current = end;
            entry = nullptr;
        }
