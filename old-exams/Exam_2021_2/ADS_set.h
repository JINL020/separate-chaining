...
class ADS_set {
   ...
   public:
    ...
    const_iterator y() const {
        ...
        for (size_type i{0}; i < buckets; ++i) {
            if (values[i].head != nullptr) {
                return const_iterator{values + i, values + buckets, values[i].head, Mode::speziell, current_size, values + index};
            }
        }
        return end();
    }
...
class ADS_set<Key, N>::Iterator {
   ...
   public:
    explicit Iterator(List *current, List *end, Entry *entry = nullptr, Mode mode = Mode::normal, size_type target = 0, List *begin = nullptr) : current{current}, end{end}, entry{entry}, mode{mode}, target{target}, begin{begin} {
    }

    Iterator() : current{nullptr}, end{nullptr}, entry{nullptr}, mode{Mode::normal}, begin{nullptr} {
    }
    ...
    //präfix!!!!!!!!!!!!!!!!!!!!!!
    Iterator &operator++() {
        if (counter == bound && bound == target - 1) {
            current = end;
            entry = nullptr;
            return *this;
        }
        ...
        /////////////
        if (mode == Mode::speziell && current != end) {
            ++counter;
        }
        return *this;
    }
